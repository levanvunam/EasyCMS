...
window.onload = function () {
    ...
    window.editor_html = CodeMirror.fromTextArea(emailCode, {
        mode: "text/html",
        lineNumbers: true,
        lineWrapping: true,
        extraKeys: {
...
jQuery(function ($) {
    ...
    $('#id-btn-preview').on('click', function () {
        ...
        var rawHtml = window.editor_html.getValue();

        var convertFrame = $('#convert-frame').get(0);
        var frameWin = convertFrame.contentWindow || convertFrame;
        var outputHtml = frameWin.convert_email(rawHtml, options);
    ...
    $('#id-btn-save').on('click', function () {
        ...
        try {
            if (objectURL) winURL.releaseObjectURL(objectURL);
        } catch (e) { }
        objectURL = winURL.createObjectURL(new Blob([outputHtml], { type: 'text/html' }));

        $('#btn-save-html')
        .attr({ 'download': 'output-email.html' })
        .get(0).href = objectURL;
