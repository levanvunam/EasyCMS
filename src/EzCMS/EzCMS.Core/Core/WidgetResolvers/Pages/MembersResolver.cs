...
{
    public class MembersResolver : Widget
    {
        #region Setup
        ...
        public override WidgetSetupModel GetSetup()
        {
            return new WidgetSetupModel
            {
                ...
                Description = "Load members for page and render using template",
                DefaultTemplate = "Default.Members",
                Type = typeof(MemberWidgetManageModel),
        ...
        [WidgetParam(Name = "PageId", Order = 1, Description = "The id of page that need to generate members")]
        ...
        public override string Render(string[] parameters)
        {
            ...
            var model = _pageService.GetMembers(PageId);

            if (model == null)
            {
                return _localizedResourceService.T("Widget_Member_Message_InvalidPageId");
            }
