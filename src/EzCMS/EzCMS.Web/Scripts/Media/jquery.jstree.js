		...
		_fn : {
			...
			load_node_json : function (obj, s_call, e_call) {
				...
				switch(!0) {
					...
					// function option added here for easier notification integration (also supporting async - see callback)
		...
		_fn : {
			save_cookie : function (c) {
				...
				if(!c) { // if called manually and not by event
					if(s.save_loaded) {
						this.save_loaded();
						$.cookie(s.save_loaded, this.data.core.to_load.join(","), s.cooky_options);
					}
					if(s.save_opened) {
						this.save_opened();
						$.cookie(s.save_opened, this.data.core.to_open.join(","), s.cooky_options);
					}
					if(s.save_selected && this.data.ui) {
						this.save_selected();
						$.cookie(s.save_selected, this.data.ui.to_select.join(","), s.cooky_options);
					}
					return;
				}
				switch(c) {
...
(function ($) {
	...
	$.jstree.plugin("search", {
		...
		_fn : {
			...
			_Text_open : function (is_callback) {
				var _this = this,
					done = true,
					current = [],
					remaining = [];
				if(this.data.search.to_open.length) {
					...
					if(current.length) {
						...
						$.each(current, function (i, val) { 
							_this.open_node(val, function () { _this._Text_open(true); }); 
						});
						done = false;
					}
				}
...
/*
* jsTree notification plugin
* This plugin gets jstree to use a class notification to retrieve data, creating great dynamism
*/
(function ($) {
	...
	$.jstree.plugin("notification", {
		__init : function () {
			if(!this.data.json_data) { throw "jsTree notification: jsTree json_data plugin not included."; }
